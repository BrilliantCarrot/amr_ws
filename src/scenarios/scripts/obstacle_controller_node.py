#!/usr/bin/env python3
"""
obstacle_controller_node_cbf_test.py

CBF 개입 검증용 동적 장애물 제어 노드.
- test_room.sdf의 obstacle_2, obstacle_4, obstacle_5를 y축으로 왕복 이동시킴
- 각 장애물은 x 고정, y=-0.25 ~ +0.25 범위에서 움직임
- A* 경로가 y=0 근처를 지날 때 장애물이 경로를 가로지르도록 만들어 CBF 개입 가능성을 높임
"""
...
class ObstacleControllerNode(Node):
    def __init__(self):
        ...
        # test_room.sdf 기준 장애물 초기 x 좌표 사용함
        self.obstacles = [
            {'name': 'obstacle_2', 'x': 1.0, 'z': 0.25, 'phase': 0.00},
            # {'name': 'obstacle_4', 'x': 3.5, 'z': 0.25, 'phase': 0.33},
            # {'name': 'obstacle_5', 'x': 5.0, 'z': 0.25, 'phase': 0.66},
        ]
        ...
        self.y_amp = 0.5 * (self.y_max - self.y_min)

        # 한 왕복 주기임. 너무 빠르면 tracker가 불안정해질 수 있어서 8~12초 권장함
        self.period = 100.0
        ...
        self.get_logger().info(
            'CBF 테스트용 obstacle controller 시작 — '
            'obstacle_2/4/5 y=-0.25~0.25 왕복 예정'
        )

    def _on_ready_callback(self):
        """Gazebo 초기화 대기 후 제어 활성화함"""
        self.ready = True
        self.ready_timer.cancel()
        self.get_logger().info('Gazebo 초기화 대기 완료 — 동적 장애물 pose 갱신 시작')

    def _set_pose_sync(self, model_name: str, x: float, y: float, z: float):
        """Ignition set_pose 서비스로 모델 pose 갱신함"""
        req = (
            f'name: "{model_name}" '
        ...
        try:
            ...
            if result.returncode != 0:
                self.get_logger().warn(
                    f'set_pose 실패: {model_name}, returncode={result.returncode}',
    ...
    def _triangle_wave(self, elapsed: float, phase_offset: float) -> float:
        """0~1 phase를 삼각파로 바꿔 y_min~y_max 왕복값 생성함"""
        phase = ((elapsed / self.period) + phase_offset) % 1.0

        if phase < 0.5:
            ratio = phase / 0.5
        else:
            ratio = 1.0 - ((phase - 0.5) / 0.5)
    ...
    def _control_callback(self):
        ...
        for obs in self.obstacles:
            y = self._triangle_wave(elapsed, obs['phase'])
            self._set_pose_sync(obs['name'], obs['x'], y, obs['z'])

        self.get_logger().info(
            'dynamic obstacles updated: '
            + ', '.join(
                f"{obs['name']}=(x={obs['x']:.2f}, y={self._triangle_wave(elapsed, obs['phase']):.2f})"
                for obs in self.obstacles
            ),
