    ...
    if use_planner_str == 'true':
        path_planner_action = TimerAction(
            ...
            actions=[
                ...
                f'-p replan_obs_dist:=0.85 '
                f'-p wp_spacing:=0.05 '
                f'-p goal_tolerance:=0.20 '
                f'-p planner_clearance_cost_radius:=0.45 '
                f'-p planner_clearance_cost_weight:=12.0 '
                f'-p smoothing_los_safety_cells:=6 '
                f'-p path_viz_republish_period_sec:=1.0 '
                f'-p planner_lidar_astar_mode:=\\\"dynamic\\\" '
                f'-p planner_replan_on_lidar_obstacles:=true '
                f'-p replan_cooldown_sec:=1.50 '
                f'-p planner_obstacle_margin:=0.06 '
                f'-p planner_min_obstacle_radius:=0.30 '
                f'-p map_update_replan_enabled:=false'
            ],
            output='screen',
            shell=False
        )
